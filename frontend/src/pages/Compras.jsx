...
const Compras = () => {
  ...
  const cargarSolicitudesConLineas =
    async (
      solicitudesBase
    ) => {
  ...
  const cargarDatos =
    async () => {
      try {
        ...
        await cargarSolicitudesConLineas(
          solicitudesData
        );
      } catch (error) {
        setError(
  ...
  const solicitudesDisponiblesCompra =
    useMemo(() => {
      const mapa =
        new Map();
  ...
  const cargarSolicitudEnOrden =
    () => {
      if (
        ...
        setError(
          "Selecciona una solicitud aprobada de Equipo Interno para compra."
        );
      ...
      if (
        ...
        setError(
          "Solo pueden cargarse solicitudes aprobadas cuyo destino sea un Equipo Interno."
        );
      ...
      const nuevosSolicitud =
        solicitudProductosNuevos.filter(
          (
            producto
          ) =>
            Number(
    ...
    async (
      ...
      try {
        ...
        setError(
          error.response?.data
            ?.message ||
            "No fue posible cargar la orden para editar"
        );
      } finally {
        setGuardando(
          false
    ...
    async (
      ...
      try {
        ...
        if (
          ...
          setError(
            "No hay una orden seleccionada para editar."
          );
        ...
        setError(
          error.response?.data
            ?.message ||
            "No fue posible guardar los cambios de la orden"
        );
      } finally {
        setGuardando(
          false
          ...
          <div className="modal-card large-modal">
            <div className="modal-header">
              <div>
                <h2>
                  {ordenEditandoId
                    ? `Editar orden #${ordenEditandoId}`
                    : "Nueva orden de compra"}
                </h2>

                <p>
                  {ordenEditandoId
                    ? "Modifica proveedor, productos, cantidades o costos mientras la orden siga en borrador."
                    : "Compra para reorden de Central o vinculada a una solicitud aprobada de Equipo Interno."}
                </p>
              </div>
